...
type ConfigurationsController struct {
	baseController
}

// URLMapping ...
func (c *ConfigurationsController) URLMapping() {
...
// @router / [post]
func (c *ConfigurationsController) Post() {
	...
	if c.Method == "fast" {
		...
		var Categorie models.ConfigurationsData
		err := json.Unmarshal(c.Ctx.Input.RequestBody, &Categorie)
		if err == nil {
			c.Data["json"] = models.AddConfigurations(&Categorie)
		} else {
			c.Data["json"] = models.MessageErrorUint64(0, err.Error())
		}
	}
...
// @router /:id [get]
func (c *ConfigurationsController) GetOne() {
	beego.Debug("GetOne")
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.ParseUint(idStr, 10, 64)
	if c.Method == "fast" {
		v := models.GetConfigurationsByIdFast(id,true)
		c.Data["json"] = v
	} else {
...
// @router / [get]
//自定义开发模式
func (c *ConfigurationsController) GetAll() {
	beego.Debug("GetAll")
	beego.Debug("GetAll")
	beego.Debug(c.Method)
	var fields []string
	...
	if c.Method == "fast" {
		v := models.GetAllConfigurationsFast(types, query, names, fields, sortby, order, page, page_size, category_ids)
		c.Data["json"] = v
	} else {
...
// @router /:id [put]
func (c *ConfigurationsController) Put() {
	beego.Debug("Put")
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.ParseUint(idStr, 10, 64)
	...
	if c.Method == "fast" {
		...
		err := json.Unmarshal(c.Ctx.Input.RequestBody, &data)
		if err == nil {
			c.Data["json"] = models.UpdateConfigurationsByIdFast(data, id)
		} else {
			c.Data["json"] = models.MessageErrorUint64(0, err.Error())
		}
	} else {
...
// @Failure 403 id is empty
// @router /:id [delete]
func (c *ConfigurationsController) Delete() {
	beego.Debug("Delete")
